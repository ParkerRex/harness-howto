...
(the `src/coding-agents/claude-code-open` submodule). It covers the CLI/TUI loop, tool execution
with permissions and sandboxing, prompt assembly, compaction, and on-disk session
storage. Paths below are relative to the `src/coding-agents/claude-code-open/` submodule.
